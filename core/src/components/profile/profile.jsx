...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const resumesRes = await axios.get('http://localhost:3001/api/resumes', {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleEditResume = (resume) => {
    navigate('/editor', { state: { resumeId: resume._id, resumeName: resume.name, resumeData: resume.data } });
  };
      ...
      <div className="card mb-4">
        <div className="card-body">
          ...
          <form onSubmit={handleUpdateProfile}>
            ...
            <div className="mb-3">
              <label htmlFor="password" className="form-label text-start d-block ">New Password</label>
              <input
                type="password"
                className="form-control"  
                id="password"
          ...
          {message && (
            <div className={`mt-3 alert ${message.includes('failed') ? 'alert-danger' : 'alert-success'}`}>
              {message}
            </div>
          )}
        </div>
